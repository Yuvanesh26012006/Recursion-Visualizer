...
export default function CodeEditor({ code, onChange, currentLine, isRunning }) {
  ...
  const isBaseCaseLine = (line) => {
    const baseCasePatterns = ['return 1', 'return 0', 'return n', 'return arr', 'if (n <= 1)', 'if (n == 0)', 'if (low > high)', 'if (n < 2)'];
    return baseCasePatterns.some(pattern => line.toLowerCase().includes(pattern.toLowerCase()));
  };

  const isRecursiveCallLine = (line) => {
    const funcNames = ['factorial', 'fibonacci', 'fib', 'binarySearch', 'search', 'solve'];
    return funcNames.some(name => {
      const regex = new RegExp(`${name}\\s*\\(`);
      return regex.test(line) && !line.includes('function') && !line.includes('int ') && !line.includes('public');
    });
  };
        ...
        <div className="flex">
          ...
          <div className="flex-shrink-0 py-4 px-2 bg-slate-800/30 select-none">
            {lines.map((_, i) => (
              <div 
                key={i} 
                className="text-xs text-slate-500 text-right pr-2 leading-6 font-mono"
                style={{ minWidth: '2rem' }}
              >
                {i + 1}
              </div>
          ...
          <div className="flex-1 py-4 overflow-x-auto">
            {lines.map((line, i) => {
              ...
              return (
                <div 
                  key={i}
                  className={`px-4 leading-6 transition-all duration-300 ${getLineHighlight(i)}`}
                >
                  <code className={`text-sm font-mono whitespace-pre ${
                    isBaseCase ? 'text-emerald-400' : 
                    isRecursive ? 'text-indigo-400' : 
                    'text-slate-300'
                  }`}>
                    {line || ' '}
                  </code>
